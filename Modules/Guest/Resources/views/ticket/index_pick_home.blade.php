        ...
        <div class="container">
            <div class="users">
                ...
                <div class="users-content">
                    <div class="tab-nav">
                        <a href="{{ route('get_guest.ticket.index_pick_home',['status' => 1]) }}"
                           class="js-tab-nav-ajax {{ $status == 1 ? 'active' : '' }}" data-id="#tab-process">Cần trung
                            chuyển</a>
                        <a href="{{ route('get_guest.ticket.index_pick_home',['status' => 2]) }}"
                           class="js-tab-nav-ajax {{ $status == 2 ? 'active' : '' }}" data-id="#tab-success">Đã trung
                            chuyển</a>
                        <a href="{{ route('get_guest.ticket.index_pick_home',['status' => 2]) }}"
                           class="js-tab-nav-ajax {{ $status == 3 ? 'active' : '' }}" data-id="#tab-success">Đã giao xe</a>
                        <a href="{{ route('get_guest.ticket.index_pick_home',['status' => -1]) }}"
                           class="js-tab-nav-ajax {{ $status == -1 ? 'active' : '' }}" data-id="#tab-cancel">Đã huỷ</a>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: left;position: relative">
                                        ...
                                        <div class="dropdown-content">
                                            @if($item->t_status_pick_home == 1)
{{--                                                <a href="{{ route('get_guest.ticket.success', $item->id) }}"> Chưa đón</a>--}}
                                                <a href="{{ route('get_guest.ticket.index_pick_home_success', $item->id) }}"> Đã đón</a>
                                                <a href="{{ route('get_guest.ticket.index_pick_home_success_car', $item->id) }}"> Đã đón và giao xe</a>
                                                <a href="{{ route('get_guest.ticket.index_pick_home_cancel', $item->id) }}"> Đã huỷ</a>
                                            @elseif ($item->t_status_pick_home == 2)
                                                <a href="{{ route('get_guest.ticket.index_pick_home_success_car', $item->id) }}"> Đã đón và giao xe</a>
                                            @endif
                                        </div>
                                    </td>
